...
let rec eval1 ctx t =
  ...
  | TmApp (_, (TmUnfold (_, tyS)), (TmApp (_, (TmFold (_, tyT)), v))) when
      isval ctx v -> v
  | TmApp (fi1, (TmFold (fi2, tyS)), t2) ->
      let t2' = eval1 ctx t2 in TmApp (fi1, TmFold (fi2, tyS), t2')
  | TmApp (fi1, (TmUnfold (fi2, tyS)), t2) ->
      let t2' = eval1 ctx t2 in TmApp (fi1, TmUnfold (fi2, tyS), t2')
